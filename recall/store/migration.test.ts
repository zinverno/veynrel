...
function v2(): RecallCardsSnapshot {
  const card = candidate();
  return { version: 2, updatedAt: 100, cards: { [card.id]: { ...card, firstSeenAt: 40, lastSeenAt: 100,
    state: "active", schedule: rateSchedule(createInitialSchedule(40), "easy", 50).schedule } } };
}

describe("Recall v1 to v2 read-only migration", () => {
  it.each(["review", "inventory"])("loads original v1 cards without IO beyond metadata; next %s writes v2", async (mutation) => {
    ...
    expect(JSON.parse(storage.bytes()!) as unknown).toMatchObject({ version: 2 });
  ...
  it("keeps strict v1 validation instead of repairing malformed legacy cards", () => {
    ...
    expect(decodeRecall('{"version":1,"updatedAt":100,"cards":{}}')).toEqual({ status: "loaded", data: { version: 2, updatedAt: 100, cards: {} } });
    expect(decodeRecall(JSON.stringify({ ...legacy, extra: true })).status).toBe("invalid");
  });
});

describe("Recall v2 scheduler validation and poisoning", () => {
  it.each([
